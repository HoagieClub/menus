...
import {
    Pane, Card, Heading, Text, majorScale,
...
class DateWidget extends Component<{}, MyState> {
    constructor(props) {
        ...
        } else if (currentHour <= 2) currentMealIndex = 0;
    ...
    moveMealLeft = () => {
        const index = this.state.currentMealIndex;
        ...
        if (index > 0) {
            ...
            const oldDate = new Date(this.state.year, this.state.month - 1,
                this.state.date);
            const newDate = new Date(oldDate.getTime() - 20 * 60 * 60 * 1000);
            if (oldDate < new Date()) { return; }
            if (days[this.state.dayIndex] === 'Sunday'
             || days[this.state.dayIndex] === 'Monday') {
                this.setState({
                    currentMealIndex: 1,
    ...
    moveMealRight = () => {
        const index = this.state.currentMealIndex;
        if (index < 2) {
            if (index === 1 && (days[this.state.dayIndex] === 'Saturday'
            || days[this.state.dayIndex] === 'Sunday')) {
                const oldDate = new Date(this.state.year,
            ...
            && (
                ...
                >
                    <Pane flex={1} alignItems="center" display="flex">
                        {(days[this.state.dayIndex] !== 'Saturday'
                        && days[this.state.dayIndex] !== 'Sunday') && (
                    ...
                    <Pane marginRight={majorScale(2)} marginTop={majorScale(2)}>
                        <Text fontSize={20} marginRight={majorScale(5)}>
                            {days[this.state.dayIndex]}, {this.state.month}
                            /{this.state.date}
                        </Text>
                        <IconButton
                            icon={ChevronLeftIcon}
                            marginRight={majorScale(5)}
                            onClick={this.moveMealLeft}
                        />
                        <IconButton
                            icon={ChevronRightIcon}
                            onClick={this.moveMealRight}
                        />
                    </Pane>
                </Card>
            )}
